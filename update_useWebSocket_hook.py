#!/usr/bin/env python3
"""
Update the useWebSocket hook to use proper backend URL configuration
🔱 JAI MAHAKAAL! Ensure WebSocket connections work with production setup
"""
import os

# Updated useWebSocket hook content
UPDATED_USE_WEBSOCKET_CONTENT = '''import { useState, useEffect, useCallback, useRef } from 'react';
...
def update_use_websocket_hook():
    """Update the useWebSocket hook with proper configuration"""
    hook_path = "/app/frontend/modern/src/hooks/useWebSocket.ts"
    
    try:
        with open(hook_path, 'w') as f:
            f.write(UPDATED_USE_WEBSOCKET_CONTENT)
        
        print(f"✅ Updated useWebSocket hook at {hook_path}")
        return True
        
    except Exception as e:
        print(f"❌ Failed to update useWebSocket hook: {e}")
        return False

if __name__ == "__main__":
    update_use_websocket_hook()